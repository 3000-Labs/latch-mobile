...
import { NetworkCongestion } from '../types/config/types';
import { stroopToXlm } from '../utils';
...
export const getIsAllowHttp = (networkUrl: string) => !networkUrl.includes('https');
...
export const getSorobanRpcServer = (network: NETWORKS) => {
  ...
  if (!sorobanRpcUrl) {
    console.log('StellarService', 'No Soroban RPC URL available for network', network);
  ...
  try {
    ...
    console.info('StellarService', 'Failed to instantiate Soroban RPC Server', {
...
export const submitTx = async (
  ...
  const transaction =
    typeof tx === 'string' ? TransactionBuilder.fromXDR(tx, networkPassphrase) : tx;
...
export const getNetworkFees = async (server: Horizon.Server) => {
  ...
  try {
    const { max_fee: maxFee, ledger_capacity_usage: ledgerCapacityUsage } = await server.feeStats();
